...
- no plaintext private-CV projections; these were retired after application-level
...
updates the encrypted PostgreSQL snapshot in one PostgreSQL transaction. Active
compatibility copies live only in system runtime storage. Closing or expiring a
workspace removes the temporary copy, not the PostgreSQL CV.
...
- Private CV operations materialize an authorized runtime copy instead of
  querying persistent plaintext projections.
...
- Public profiles and future consented aggregate features have a conventional
  ownership model.
- PostgreSQL dumps and persistent queued-upload storage do not contain plaintext
  private CV contents; public profiles and operational account metadata remain
  readable by design.
